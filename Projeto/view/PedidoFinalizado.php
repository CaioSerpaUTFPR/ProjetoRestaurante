    ...
    <div class="container">
        <div class="row">
            <table class="table">
                <thead>
                    <thead >
                        <h1 style="text-align: center; width: 100%; background-color: #d3d3d3">Pedidos Finalizados</h1>
                    </thead>
                ...
                <?php
                    $resultado = $pedido->getPedidosFinalizado();
                    ...
                    while($result = $resultado->fetch(PDO::FETCH_OBJ)){
                        ...
                        while($auxResult = $auxResultado->fetch(PDO::FETCH_OBJ)){
                            ...
                            $precototal+=$auxResult->preco;
                        }
                        echo "</td>"; 
                        echo "<td>R$ $precototal</td>";
                        if($_SESSION['cargo'] != 2){
                            if($result->idStatus == 3){
                                echo "<td><a href=../controller/pedidoController.php?entregar=$result->idPedido class='btn btn-danger'>Entregar Pedido</a></td>";
                            }else{
                                echo "<td><button type='button' disabled class='btn btn-danger'>Pedido Entregue</button></td>";
                            }
                        }                  
                    }
                    ?>            
